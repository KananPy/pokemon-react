...
export class PokemonGame extends Component {
  ...
  render() {
    ...
    const team1Point = this.state.team1.reduce((sum, pokemon) => sum+pokemon.base_experience, 0)
    const team2Point = this.state.team2.reduce((sum, pokemon) => sum+pokemon.base_experience, 0)


    return (
      <div style={{display:"flex", flexDirection: "column", alignItems: "center"}}>
        <h1 className={styles.playTitle}>Welcome to The Pokemon Game</h1>
        <button onClick={this.onHandlePlay} className={styles.playButton}>Play</button>

        {this.state.isShowPanel && ( <div>
          <PokemonTeam
            teamName="Team1"
